...
void DrawingHistograms::Filling_Data_for_SubDetectors(int subdetector_iterator) {
  ...
  for (auto iterator = HitMap.begin(); iterator != HitMap.end(); ++iterator) {
    //Phill has commented out the original code and replaced it with something else for now
    int const layer_number = SubDetectors->at(subdetector_iterator)->GetLayer(iterator->first);
    int const total_cell_number = SubDetectors->at(subdetector_iterator)->GetTotCellNumber();
    int const all_hits_per_cell = iterator->second;
    int const cell_id = iterator->first;
    int const unhit_cells = total_cell_number - HitMap.size();

    Bufferdepth_Histo_.at(layer_number)->Fill(all_hits_per_cell);
    Bufferdepth_Histo_.at(layer_number)->SetBinContent(1,total_cell_number);
    Bufferdepth_Histo_.at(MaxNumberLayers+1)->Fill(all_hits_per_cell); 
    Bufferdepth_Histo_.at(MaxNumberLayers+1)->SetBinContent(1,unhit_cells);
...
void DrawingHistograms::Filling_Data_of_hits(int file_iterator, int subdetector_iterator, Data* data,
  ...
  unsigned long long int const CellIDkey = SubdetectorCells->CellID_ToLONGconversion(SubdetectorCells->GetCellID());
...
gErrorIgnoreLevel=kFatal;
  ...
  for (signed int l = 0; l < hitLayers.size(); ++l) {
    ...
    Hits_Canvas_->SetLogz(0);
		...
		AdditionalText = "Fraction of cells that are not hit: ";

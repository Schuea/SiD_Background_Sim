...
public:
  ...
  std::map< int, std::pair< std::vector< int >, float > > Get_AverageOccupancy_Phi() const;
  ...
  void Calculate_Average( std::map< int, std::pair< std::vector< int >, float > > & map);
...
protected:
  ...
  std::map< int, std::pair< std::vector< int >, float > > AverageOccupancy_Phi;
  int BunchNumber;
};

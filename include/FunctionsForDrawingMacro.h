...
template <class T>
void WritePrintComparedHistogram(TCanvas* Canvas_, std::vector<T> Histos_, std::string new_histo_title, std::vector< int > hitLayers, int firstlayer, int lastlayer, bool Normalizing, std::string drawingoption, std::string PDFName){
  Canvas_->Update();
  ...
  CanvasName_eps << Canvas_->GetName() << "_" << Histos_.at(firstlayer)->GetName() << "_" << firstlayer << "-" << lastlayer << ".eps";
  CanvasName_C << Canvas_->GetName() << "_" << Histos_.at(firstlayer)->GetName() << "_" << firstlayer << "-" << lastlayer << ".C";
...
template <class T>
void WritePrintComparedHistogram(TCanvas* Canvas_, T Histo, int firstlayer, int lastlayer, std::string drawingoption, std::string PDFName){
  Canvas_->Update();
...
template <class T>
void Fill_Histogram_from_Map (std::map< std::pair< int, int >, std::vector< float > > HitMap, std::vector<T> *Hits, int weight) {
  ...
  if (SubDetectors->size() == 1) {
    ...
    *axis_range_plot_1D = temp1D;
    ...
    std::vector<float> tempenergy1D = { 
      ...
      SubDetectors->at(0)->GetROOTEnergyHisto_binning().at(1), SubDetectors->at(0)->GetROOTEnergyHisto_binning().at(2) };
...
void SetupHistoTitles(std::string subdetector_name, std::string layer,
  ...
  *(particleoriginshisto_title) = "Origins of std::pair background particles for " + subdetector_name + " layer " + layer;
}
...
void Setup_ParticleOriginsHisto(std::vector<TH2D*> HistoVector, std::vector<float> axis_range_plot, std::string histo_name, std::string histo_title, std::string coordinate_system) {
  if(axis_range_plot.size()<9) {
    std::cerr << "The plot of the particle origins can only be plotted with the std::vector of the 3 dimensional plot axes." << std::endl;
    std::terminate();
  }
  ...
  if (coordinate_system == std::string("spherical")){
    float rmax = sqrt(pow(axis_range_plot.at(5),2)+pow(axis_range_plot.at(8),2)+pow(axis_range_plot.at(2),2));
    float rmin = 0.;
    float rrange = rmax - rmin;
    float zmax = axis_range_plot.at(2);
    float zmin = -zmax;
  ...
  HistoVector.emplace_back(
      new TH2D(histo_name.c_str(), histo_title.c_str(), axis_vector.at(3),
...
void Setup_Histo(std::vector<TH2D*> HistoVector, std::vector<float> axis_range_plot, std::string histo_name, std::string histo_title) {
  HistoVector.emplace_back(
      new TH2D(histo_name.c_str(), histo_title.c_str(), axis_range_plot.at(0),
...
void Setup_Histo(std::vector<TH3D*> HistoVector, std::vector<float> axis_range_plot, std::string histo_name, std::string histo_title) {
  HistoVector.emplace_back(
      new TH3D(histo_name.c_str(), histo_title.c_str(), axis_range_plot.at(0),
        axis_range_plot.at(1), axis_range_plot.at(2), axis_range_plot.at(3), axis_range_plot.at(4),

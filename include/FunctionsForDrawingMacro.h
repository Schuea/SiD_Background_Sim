...
template<class T> void WritePrintComparedHistogram(TCanvas* Canvas_, std::vector<T> Histos_,
		std::string new_histo_title, std::vector<int> hitLayers, int firstlayer, int lastlayer, bool Normalizing,
		std::string drawingoption, std::string PDFName);
template<class T> void WritePrintComparedHistogram(TCanvas* Canvas_, T Histo, int firstlayer, int lastlayer,
		std::string drawingoption, std::string PDFName);
template<class T> void WritePrintHistogram(TCanvas* Canvas_, T Histos_, std::string drawingoption,
		std::string AdditionalText, std::string PDFName);
template<class T> void WritePrintHistogram(TCanvas* Canvas_, T Histos_, bool Normalizing, std::string drawingoption,
		std::string AdditionalText, std::string PDFName);
...
template<class T> void Fill_Histogram_from_Map(std::map<std::pair<int, int>, std::vector<float> > HitMap,
		std::vector<T> *Hits, int weight);
...
template<class T>
void WritePrintHistogram(TCanvas* Canvas_, T Histos_, std::string drawingoption, std::string AdditionalText,
		std::string PDFName) {
	Canvas_->Update();
	Canvas_->SetRightMargin(0.15);
	Histos_->Draw(drawingoption.c_str());

	TText *Bufferdepth_label = new TText();
	Bufferdepth_label->SetNDC();
	Bufferdepth_label->SetTextFont(1);
	Bufferdepth_label->SetTextColor(1);
	Bufferdepth_label->SetTextSize(0.03);
	Bufferdepth_label->SetTextAlign(22);
	Bufferdepth_label->SetTextAngle(0);
	Bufferdepth_label->DrawText(0.3, 0.4, AdditionalText.c_str());
...
template<class T>
void WritePrintHistogram(TCanvas* Canvas_, T Histos_, bool Normalizing, std::string drawingoption,
		std::string AdditionalText, std::string PDFName) {
	Canvas_->Update();
	Canvas_->SetRightMargin(0.15);
	if (Normalizing) {
		...
		Histos_->SetMinimum(0.00000000001);
	}
	Histos_->Draw(drawingoption.c_str());
...
void Fill_Histogram_from_Map(std::map<std::pair<int, int>, std::vector<float> > HitMap, std::vector<T> *Hits,
	...
	for (int bin_iterator = 0; bin_iterator < totalbins; ++bin_iterator) {
		if (std::find(overflowbins.begin(), overflowbins.end(), bin_iterator) != overflowbins.end())
			continue;
